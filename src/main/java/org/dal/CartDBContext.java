...
public class CartDBContext extends DBContext{
    public ArrayList<Cart> getProductByCart(int accountId) {
        String sql = "Select p.id AS product_id, p.name, p.price, p.image_url, c.account_id, c.quantity AS cartQuantity ,p.quantity AS productQuantity From Cart c JOIN Product p ON c.product_id = p.id Where account_id = ?";
